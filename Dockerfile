...
WORKDIR $GOPATH/src/github.com/oikuyinminu/ghr

COPY go.mod go.sum $GOPATH/src/github.com/oikuyinminu/ghr/

RUN go mod tidy

COPY . .

RUN go build -ldflags="-s -w" -a -o /ghr
